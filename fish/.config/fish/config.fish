...
function fish_prompt
    ...
    if test -n "$git_branch"
        echo -n (set_color cyan)""(set_color brwhite -b cyan)" $username "(set_color cyan -b blue)""(set_color brwhite -b blue)" $current_dir "(set_color blue -b brblack)""(set_color brwhite -b brblack)" git: $git_branch "(set_color brblack -b normal)""(set_color bryellow -b normal)" \$ "(set_color normal)
    else
